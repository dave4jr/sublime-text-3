...
def checkout(request, template, pk):
	...
	if request.method == "POST":
		...
		checkout_form = CheckoutForm(instance=reservation, initial={"checkout_datetime_actual":reservation.checkout_datetime, "checkout_mileage": reservation.bike.current_mileage})
		
	return render(request, template, {"reservation":reservation, "CheckoutForm": checkout_form } )
...
def checkout_condition(request, template, pk):
	...
	if request.method == "POST":
		...
		# condition = request.POST.getlist("condition")[0]
		# data={"checkout_signature":signature, "checkout_condition": condition}
		reservation.checkout_signature = request.POST.getlist("signature")[0]
		reservation.checkout_condition = request.POST.getlist("condition")[0]

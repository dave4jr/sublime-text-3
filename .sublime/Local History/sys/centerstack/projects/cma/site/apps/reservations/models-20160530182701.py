...
#	Choices & Functions
# ========================= #
preference = Preferences.objects.get(pk=1)
...
class Workflow(wf.Workflow):
	...
	transitions = (
		('send_deposit', 'submitted', 'pending_deposit'),
		('receive_deposit', 'pending_deposit', 'reserved'),
		('checkout', 'reserved', 'checked_out'),
		('checkin', 'checked_out', 'checked_in'),
		('no_damage', 'checked_in', 'closed'),
		('yes_damage', 'checked_in', 'damaged'),
		('send_damage_invoice', 'damaged', 'pending_damage_payment'),
		('receive_damage_payment', 'pending_damage_payment', 'closed'),
		('reset', ("pending_deposit", "reserved", "checked_out", "checked_in", "damaged", "pending_damage_payment", "closed"), 'submitted'),
	)
...
# ==================================================#
	...
	def bike_insurance(self):
		if self.bike is None:
			...
			return (self.bike.insurance_price * self.duration)
	...
	def storage_price(self):
		if self.rental_type == "storage":
			months = monthdiff(self.checkout_datetime, self.checkin_datetime)
			return int(round(90 * months))
		else:
			return 0

...
from bikes.models import Accessories
...
# ========================= #
try:
	preference, created = Preference.objects.get_or_create(pk=1)
except:
	pass
YES_NO_CHOICES = (("Yes", "Yes"), ("No", "No"))
TYPE_CHOICES = (("bike", "Bike"), ("class", "Class"), ("class_bike", "Class / Bike"), ("custom", "Custom"), ("custom_bike", "Custom / Bike"), ("storage", "Storage"), ("tour", "Tour"), ("tour_bike", "Tour / Bike"))
...
STORAGE_BIKE_NUM_CHOICES = ((1, "1 Bike"),(2, "2 Bikes"),(3, "3 Bikes"),(4, "4 Bikes"),(5, "5 Bikes"))
...
class Reservation(wf.WorkflowEnabled, models.Model):
	...
	def get_event(self):
		if self.classes is None:
			return "---"
		elif self.rental_type == "class" or self.rental_type == "class_bike":
	...
	def get_accessories(self):
		return ", ".join([str(ii.name) for ii in self.accessories.all()])
	get_accessories.short_description = "Accessories"

	def get_accessories_price(self):
		accessories_sum = []
		try:
			for ii in self.accessories.all():
				if ii.price_period == "One-Time":
					accessories_sum.append(ii.price)
				elif ii.price_period == "Daily":
					accessories_sum.append(ii.price * self.duration)
				else:
					pass
		except:
			return "---"
		return sum(accessories_sum)
...
#	Storage
# ==================================================#
	storage_bike_num = models.CharField(max_length=15, null=True, blank=True, choices=STORAGE_BIKE_NUM_CHOICES, default=1)
		
	storage_bike_1 = models.CharField(max_length=50, null=True, blank=True)
	storage_bike_2 = models.CharField(max_length=50, null=True, blank=True)
	storage_bike_3 = models.CharField(max_length=50, null=True, blank=True)
	storage_bike_4 = models.CharField(max_length=50, null=True, blank=True)
	storage_bike_5 = models.CharField(max_length=50, null=True, blank=True)
	
	
	def storage_months(self):
		if self.rental_type == "storage":
			return monthdiff(self.checkout_datetime, self.checkin_datetime)
		else:
			return 0
	storage_months = property(storage_months)


	def storage_price_total(self):
		if self.rental_type == "storage":
			return (int(self.storage_months) * preference.storage_price * int(self.storage_bike_num))
		else:
			return 0
	storage_price_total = property(storage_price_total)
...
#	Checkout
# ==================================================#
	checkout_condition_diagram = models.TextField(blank=True, null=True)
	checkout_condition_img_1 = models.FileField(upload_to='Check-Out/%Y/%m/', blank=True, null=True)
	checkout_condition_img_2 = models.FileField(upload_to='Check-Out/%Y/%m/', blank=True, null=True)
	checkout_condition_img_3 = models.FileField(upload_to='Check-Out/%Y/%m/', blank=True, null=True)
	checkout_condition_img_4 = models.FileField(upload_to='Check-Out/%Y/%m/', blank=True, null=True)
	checkout_condition_img_5 = models.FileField(upload_to='Check-Out/%Y/%m/', blank=True, null=True)
	checkout_condition_img_6 = models.FileField(upload_to='Check-Out/%Y/%m/', blank=True, null=True)
	checkout_condition_signature_renter = models.TextField(blank=True, null=True)
	checkout_condition_signature_dealer = models.TextField(blank=True, null=True)
	checkout_liability_signature = models.TextField(blank=True, null=True)
	checkout_membership_signature = models.TextField(blank=True, null=True)
...
#	Checkin
# ==================================================#
	checkin_datetime_actual = models.DateTimeField(verbose_name="Check-In (Actual)", blank=True, null=True, default=checkin_datetime)
	...
	def trip_mileage(self):
		return self.checkin_mileage - self.checkout_mileage
	trip_mileage = property(trip_mileage)
...
# ==================================================#
	...
	def bike_insurance(self):
		if self.bike is None:
			...
			return (preference.insurance_high_deductible_price * self.duration)

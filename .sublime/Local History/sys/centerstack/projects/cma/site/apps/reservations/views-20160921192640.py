...
from .forms import CheckoutAgreementForm
...
def checkout_condition(request, template, pk):
	...
	if request.method == "POST":
		...
		if checkout_condition_form.is_valid() and checkout_condition_bike_form.is_valid():
			...
			checkout_condition.checkout_condition_signature_renter = request.POST.getlist("signature_renter")[0]
			checkout_condition.checkout_condition_signature_dealer = request.POST.getlist("signature_dealer")[0]
...
def checkout_liability(request, template, pk):
	...
	if request.method == "POST":
		checkout_liability_form = CheckoutLiabilityForm(request.POST, instance=reservation)
		if checkout_liability_form.is_valid():
			checkout_liability = checkout_liability_form.save(commit=False)
			checkout_liability.checkout_liability_signature = request.POST.getlist("signature")[0]
			checkout_liability.checkout_liability = request.POST.getlist("liability")[0]
			checkout_liability.save()
			return redirect("checkout", pk=pk)
	else:
		checkout_liability_form = CheckoutLiabilityForm(instance=reservation)
		
	return render(request, template, {"reservation":reservation, "CheckoutLiabilityForm": checkout_liability_form } )





def checkout_agreement(request, template, pk):
	reservation = Reservation.objects.get(pk=pk)

	if request.method == "POST":
		form = CheckoutAgreementForm(request.POST, instance=reservation)
		if form.is_valid():
			form.save()
			return redirect("checkout", pk=pk)
	else:
		form = CheckoutAgreementForm(instance=reservation)
		
	return render(request, template, {"reservation":reservation, "CheckoutAgreementForm": form } )

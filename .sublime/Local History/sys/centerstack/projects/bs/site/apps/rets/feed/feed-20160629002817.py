...
for offset in offset_range:
	...
	#	Search
	# ====================================== #
	SELECT_LIST = HEADER = ["Acres","Area","Bathrooms","Bedrooms","City","RESICOMM","County","RESIEXTE","RESIHTCO","RESIHOA","RESIHOAP","RESIHOAD","RESIINC1","RESIINC2","RESIINC3","RESIINTE","RESIKITC","Latitude","ListingAgentFullName","ListingDate","MLNumber","ListingPrice","ListingRid","Longitude","MarketingRemarks","RESIPARK","PropertyType","Region","RESIROOM","Style","SquareFootage","State","Status","StatusDate","StreetName","StreetNumber","StreetSuffix","RESISTYL","Unit","RESIVIEW","YearBuilt","ZipCode","RESIWTRD","RESILEVL"]
...
for house in JSON:
	...
	for key, value in house.iteritems():
		...
		if field == "img":
			imgs = list(value)
			for img in imgs:
				image_db, created = Image.objects.update_or_create(image_id=str(imageid), defaults={"img":img})
				image_db.save()
				imageid += 1
		else:

...
# class PropertyCron(CronJobBase):
#     	RUN_EVERY_MINS = 1440
#     	schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
#     	code = 'rets.property_cron'

def do():
	JSON = []
	COUNT = 0
	offset_range = ["1", "1001","2001","3001","4001"]
	...
	for offset in offset_range:
		...
		params = {
			...
			'Count': '0',
			'Offset': offset,
			'Query': "((MLNumber=1+),(Status=|A))|((MLNumber=1+),(LastModifiedDateTime=2015-07-01T00:00:00-2016-07-01T00:00:00),(Status=|A,P,S))",
			'SearchType': "property",
			'QueryType': 'DMQL2',
			'Class': "RESI",
		}
	...
	propertyid = 1
 	for house in JSON:
		...
		for key, value in house.iteritems():
			...
			if field in ["bedrooms", "bathrooms", "acres", "latitude", "longitude"]:
				defaults[field] = float(value)
			elif field in ["squarefootage", "listingprice"]:
				...
				defaults["style"] = value
			elif field == "propertysubtype1":
				defaults["subtype1"] = value
			elif field == "propertysubtype2":
				defaults["subtype2"] = value
			elif field == "propertysubtype3":
				defaults["subtype3"] = value
			else:
				defaults[field] = value
		defaults["keysearch"] = str(defaults["listingrid"]) + str(defaults["listingprice"]) + str(defaults["city"]) + str(defaults["county"]) + str(defaults["listingagentfullname"]) + str(defaults["listingdate"]) + str(defaults["marketingremarks"]) + str(defaults["mlnumber"]) + str(defaults["propertytype"]) + str(defaults["subtype1"]) + str(defaults["subtype2"]) + str(defaults["subtype3"]) + str(defaults["resicomm"]) + str(defaults["resiexte"]) + str(defaults["resihoa"]) + str(defaults["resihoad"]) + str(defaults["resihoap"]) + str(defaults["resihtco"]) + str(defaults["resiinc1"]) + str(defaults["resiinc2"]) + str(defaults["resiinc3"]) + str(defaults["resiinte"]) + str(defaults["resikitc"]) + str(defaults["resilevl"]) + str(defaults["resipark"]) + str(defaults["resiroom"]) + str(defaults["style"]) + str(defaults["view"]) + str(defaults["resiwtrd"]) + str(defaults["state"]) + str(defaults["status"]) + str(defaults["statusdate"]) + str(defaults["streetname"]) + str(defaults["streetnumber"]) + str(defaults["streetsuffix"]) + str(defaults["section"]) + str(defaults["unit"]) + str(defaults["yearbuilt"]) + str(defaults["zipcode"])
...
# ====================================== #
def delete_all():
	while Property.objects.count():
		ids = Property.objects.values_list('pk', flat=True)[:100]
		Property.objects.filter(pk__in = ids).delete()


def check_duplicates():
	duplicates = Property.objects.values('mlnumber').annotate(Count('id')).order_by().filter(id__count__gt=1)
	print duplicates
check_duplicates()

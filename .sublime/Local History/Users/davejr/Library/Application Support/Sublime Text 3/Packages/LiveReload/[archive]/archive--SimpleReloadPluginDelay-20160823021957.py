...
class SimpleRefreshDelay(LiveReload.Plugin, sublime_plugin.EventListener):
    ...
    title = 'Simple Reload with delay(1000ms)'
    description = 'Wait 1000ms then refresh page, when file is saved'
    file_types = '*'

    def on_post_save(self, view):
        ref = self
        sublime.set_timeout(lambda : ref.refresh(os.path.basename(view.file_name())), 1000)
